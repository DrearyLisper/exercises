...
daysTo from to = (toI - fromI + range) `mod` range
  where
    fromI = fromEnum from
    toI = fromEnum to
...
instance Semigroup (List1 a) where
  (<>) (List1 headA restA) (List1 headB restB) = List1 headA (restA <> [headB] <> restB)
...
instance Foldable List1 where
  ...
  foldr f start (List1 v vs) = foldr f (f v start) vs

instance Foldable Treasure where
  foldMap :: Monoid m => (a -> m) -> Treasure a -> m
  foldMap _ NoTreasure = mempty 
  foldMap f (SomeTreasure v) = f v
...
instance Functor List1 where
  ...
  fmap f (List1 v vs) = List1 (f v) (fmap f vs)

instance Functor Treasure where
  fmap :: (a -> b) -> Treasure a -> Treasure b
  fmap _ NoTreasure = NoTreasure
  fmap f (SomeTreasure v) = SomeTreasure $ f v
