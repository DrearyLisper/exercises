...
lowerAndGreater n list = strN ++ " is greater than " ++ lower ++ " elements and lower than " ++ greater ++ " elements"
    where
        lower = show $ length $ filter (<n) list
        greater = show $ length $ filter (>n) list
        strN = show n
